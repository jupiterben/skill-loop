...
**追加 Bug 反例（Story 未完成 → 写入 AC；已完成 → 新建修复 Story）：**
```bash
pnpm loop bug US-003 "不应出现的现象描述"
pnpm loop bug US-003 "描述" --ready --change-note "测试发现"
```
...
- 状态**只通过 loop-cli** 读写，禁止手改 `loop-data/`
